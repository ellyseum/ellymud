...
export class WaveCommand implements Command {
    ...
    this.clients.forEach((c) => {
      if (c.authenticated && c.user && c.user.currentRoomId === currentRoomId) {
        if (c === client) {
          ...
          writeFormattedMessageToClient(
            c,
            colorize(`${formatUsername(username)} waves.\r\n`, 'cyan')
          );
        }
      }
    });
  ...
  private waveAtTarget(
    ...
    if (!target || !target.user) {
      writeToClient(
        client,
        colorize(`Wave at whom? ${this.formatTargetName(targetName)} is not here.\r\n`, 'yellow')
      );
      return;
    }
    ...
    this.clients.forEach((c) => {
      if (c.authenticated && c.user && c.user.currentRoomId === currentRoomId) {
        if (c === client) {
          writeMessageToClient(
            c,
            colorize(`You wave at ${formatUsername(targetUsername)}.\r\n`, 'green')
          );
        } else if (c === target) {
          writeFormattedMessageToClient(
            c,
            colorize(`${formatUsername(username)} waves at you.\r\n`, 'cyan')
          );
        } else {
          writeFormattedMessageToClient(
            c,
            colorize(
              `${formatUsername(username)} waves at ${formatUsername(targetUsername)}.\r\n`,
              'cyan'
            )
          );
        }
