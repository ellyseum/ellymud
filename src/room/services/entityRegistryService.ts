...
export class EntityRegistryService implements IEntityRegistryService {
  ...
  public findClientByUsername(username: string): ConnectedClient | undefined {
    for (const client of this.getClients().values()) {
  ...
  public lookAtEntity(client: ConnectedClient, entityName: string): boolean {
    ...
    if (!matchingNPC) {
      ...
      for (const npc of npcsInRoom) {
        if (
          npc.name.toLowerCase() === normalizedName ||
    ...
    if (matchingNPC) {
      ...
      if (matchingNPC.description) {
        ...
        writeToClient(
          client,
          colorize(`It's a ${matchingNPC.name} in the room with you.\r\n`, 'cyan')
        );
      }
    ...
    if (matchingItemInstance && matchingItemTemplate) {
      ...
      if (template) {
        ...
        if (template.type === 'weapon') {
          writeToClient(
            client,
            colorize(`It's a weapon with ${template.stats?.attack || 0} attack power.\r\n`, 'cyan')
          );
        } else if (template.type === 'armor') {
          writeToClient(
            client,
            colorize(`It's armor with ${template.stats?.defense || 0} defense.\r\n`, 'cyan')
          );
        }
        ...
        if (template.requirements) {
          ...
          if (template.requirements.strength)
            reqs.push(`strength ${template.requirements.strength}`);
          if (template.requirements.dexterity)
            reqs.push(`dexterity ${template.requirements.dexterity}`);
        ...
        if (instance && instance.history && instance.history.length > 0) {
          ...
          writeToClient(
            client,
            colorize(
              `This item was most recently ${recentHistory.event}${recentHistory.details ? ': ' + recentHistory.details : ''}.\r\n`,
              'yellow'
            )
          );
        }
    ...
    const objectMatch = room.items.find((item) => {
      ...
      return (
        itemName.toLowerCase() === normalizedName || itemName.toLowerCase().includes(normalizedName)
      );
    });
    ...
    if ((normalizedName === 'gold' || normalizedName.includes('gold')) && room.currency.gold > 0) {
      ...
      writeToClient(
        client,
        colorize(`There are ${room.currency.gold} gold pieces on the ground.\r\n`, 'cyan')
      );
      ...
      (normalizedName === 'silver' || normalizedName.includes('silver')) &&
      room.currency.silver > 0
    ) {
      writeToClient(client, colorize(`You look at the silver pieces.\r\n`, 'cyan'));
      writeToClient(
        client,
        colorize(`There are ${room.currency.silver} silver pieces on the ground.\r\n`, 'cyan')
      );
      ...
      (normalizedName === 'copper' || normalizedName.includes('copper')) &&
      room.currency.copper > 0
    ) {
      writeToClient(client, colorize(`You look at the copper pieces.\r\n`, 'cyan'));
      writeToClient(
        client,
        colorize(`There are ${room.currency.copper} copper pieces on the ground.\r\n`, 'cyan')
      );
    ...
    const playerMatch = room.players.find(
      (player) =>
        player.toLowerCase() === normalizedName || player.toLowerCase().includes(normalizedName)
    );
    ...
    if (playerMatch) {
      ...
      if (playerMatch.toLowerCase() === client.user.username.toLowerCase()) {
        writeToClient(
          client,
          colorize(`You look at yourself. You look... like yourself.\r\n`, 'cyan')
        );
      ...
      for (const otherPlayerName of room.players) {
        if (
          otherPlayerName.toLowerCase() === client.user.username.toLowerCase() ||
        ...
        if (otherClient) {
          writeFormattedMessageToClient(
            otherClient,
            colorize(
              `${formatUsername(client.user.username)} looks ${formatUsername(playerMatch)} up and down.\r\n`,
              'cyan'
            )
          );
        }
    ...
    if (client.user.inventory && client.user.inventory.items) {
      ...
      const matchingInventoryItemId = client.user.inventory.items.find(
        (instanceId) => instanceId === entityName || instanceId.includes(entityName)
      );
      ...
      if (matchingInventoryItemId) {
        ...
        if (instance) {
          ...
          if (template) {
            ...
            writeToClient(
              client,
              colorize(`You look at the ${template.name} in your inventory.\r\n`, 'cyan')
            );
            writeToClient(client, colorize(`${template.description}\r\n`, 'cyan'));
            ...
            if (template.type === 'weapon') {
              writeToClient(
                client,
                colorize(
                  `It's a weapon with ${template.stats?.attack || 0} attack power.\r\n`,
                  'cyan'
                )
              );
            } else if (template.type === 'armor') {
            ...
            if (instance.history && instance.history.length > 0) {
              ...
              writeToClient(
                client,
                colorize(
                  `- ${new Date(firstEvent.timestamp).toLocaleString()}: ${firstEvent.event} ${firstEvent.details || ''}\r\n`,
                  'cyan'
                )
              );
              ...
              if (instance.history.length > 1) {
                writeToClient(
                  client,
                  colorize(
                    `- ${new Date(lastEvent.timestamp).toLocaleString()}: ${lastEvent.event} ${lastEvent.details || ''}\r\n`,
                    'cyan'
                  )
                );
              }
      ...
      if ((normalizedName === 'gold' || normalizedName.includes('gold')) && currency.gold > 0) {
        ...
        writeToClient(
          client,
          colorize(`You have ${currency.gold} gold pieces in your pouch.\r\n`, 'cyan')
        );
        ...
        writeToClient(
          client,
          colorize(`You have ${currency.silver} silver pieces in your pouch.\r\n`, 'cyan')
        );
        ...
        writeToClient(
          client,
          colorize(`You have ${currency.copper} copper pieces in your pouch.\r\n`, 'cyan')
        );
