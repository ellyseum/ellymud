document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('confirm-kick').addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const kickedPlayerElement = document
          .querySelector(`#collapse-${kickClientId}`)
  ...
  async function fetchServerStats() {
    try {
      ...
      if (data.success) {
        ...
        document.getElementById('memory-heap-total').textContent = formatBytes(
          stats.memoryUsage.heapTotal
        );
        document.getElementById('memory-heap-used').textContent = formatBytes(
          stats.memoryUsage.heapUsed
        );
        document.getElementById('memory-external').textContent = formatBytes(
          stats.memoryUsage.external
        );
      }
    } catch (error) {
  ...
  function updateBlockInputButtonState() {
    ...
    if (blockButton) {
      ...
      if (isUserInputBlocked) {
        ...
        blockButton.setAttribute(
          'data-bs-original-title',
          'Prevent the user from entering commands'
        );
      }
  ...
  async function fetchMUDConfiguration() {
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        throw new Error(
          `Expected JSON but got ${contentType || 'unknown content type'}: ${text.substring(0, 100)}...`
        );
      }
      ...
      if (data.success) {
        ...
        document.getElementById('allow-registration').checked =
          config.advanced.allowRegistration || false;
  ...
  function populatePlayersTable(players) {
    ...
    const connectedPlayers = Array.from(
      document.querySelectorAll('#player-accordion .accordion-item')
    )
      .map((item) => {
    ...
    players.sort((a, b) => {
      ...
      const aOnline = connectedPlayers.some(
        (p) => p.username.toLowerCase() === a.username.toLowerCase()
      );
      const bOnline = connectedPlayers.some(
        (p) => p.username.toLowerCase() === b.username.toLowerCase()
      );
    ...
    players.forEach((player) => {
      ...
      const isOnline = connectedPlayers.some(
    ...
    try {
      ...
      if (data.success && data.player) {
        ...
        if (template && detailForm) {
          ...
          detailForm.querySelector('.player-detail-inventory').value = JSON.stringify(
            player.inventory,
            null,
            2
          );
  ...
  document.getElementById('confirm-delete')?.addEventListener('click', async () => {
    try {
      ...
      if (data.success) {
        ...
        const deletePlayerModal = bootstrap.Modal.getInstance(
  ...
  function displayPipelineMetrics(data) {
    ...
    document.getElementById('pipeline-success-rate').textContent =
    ...
    stages.forEach((stage) => {
      ...
      if (stats) {
        document.getElementById(`${stage}-avg-duration`).textContent = stats.avgDuration
          ? `${stats.avgDuration.toFixed(1)} min`
          : '-';
        document.getElementById(`${stage}-avg-grade`).textContent = stats.avgScore
          ? scoreToGrade(stats.avgScore)
          : '-';
        ...
        const outcomeClass =
          exec.outcome === 'success'
            ...
            : exec.outcome === 'failure'
              ...
              : 'text-warning';
        const outcomeIcon =
          exec.outcome === 'success' ? '✅' : exec.outcome === 'failure' ? '❌' : '⚠️';
  ...
  document
    .getElementById('refresh-pipeline-metrics')
    ?.addEventListener('click', loadPipelineMetrics);
});
